...
Count_file <- "/Users/changzoe/Downloads/2-Single_Cell_RNAseq_Pipeline/Input/Count_File_Example_GSE116256_AML921A-D0.txt.gz"

# Define path to clinical/meta data (OPTIONAL)
meta_file <- "/Users/changzoe/Downloads/2-Single_Cell_RNAseq_Pipeline/Input/Meta_File_Example_GSE116256_AML921A-D0.txt.gz"

# Specify the output folder path
output_folder <- "/Users/changzoe/Downloads/2-Single_Cell_RNAseq_Pipeline/Output/Single_Cell_RNAseq_Output/"
...
patientID <- rownames(seurat_obj@meta.data)
r1 = do.call(rbind, strsplit(patientID, "\\."))
r1 <- as.matrix(r1)

seurat_obj@meta.data$patientID = r1[,1]
head(seurat_obj@meta.data)
agg = AggregateExpression(seurat_obj, return.seurat = T, group.by = c('CellType', 'Phase', 'patientID'), normalization.method = "LogNormalize",scale.factor = 10000)
...
colnames(meta_text) = c("CellType", "Phase", "patientID")
head(meta_text)

#treat_celltype = paste(meta_text[, "Phase"], meta_text[,"CellType"], sep="_")
