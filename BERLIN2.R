...
## Seurat H5 file
h5_file <- "/Users/changzoe/Downloads/2-Single_Cell_RNAseq_Pipeline/Output/Single_Cell_RNAseq_Output/GSE116256_AMLscRNA_h5friendly.h5seurat"

## Specify the output folder path
output_folder <- "/Users/changzoe/Downloads/2-Single_Cell_RNAseq_Pipeline/Output/Single_Cell_PostProcessing_Output/"
...
for (i in names(subset_seurat_obj@assays)) {
  ...
  scaled_counts <- as.data.frame(subset_seurat_obj@assays[[i]]@layers$scale.data)
  raw_counts <- as.data.frame(subset_seurat_obj@assays[[i]]@layers$counts)
  normalized_data <- as.data.frame(subset_seurat_obj@assays[[i]]@layers$data)
