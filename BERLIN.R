...
seurat_obj <- doubletFinder(seurat_obj, pN = 0.25, pK = 0.09, nExp = nExp, PCs = 1:10)
...
for (i in names(seurat_obj@assays)) {
  ...
  scaled_counts <- as.data.frame(seurat_obj@assays[[i]]@layers$scale.data)
  raw_counts <- as.data.frame(seurat_obj@assays[[i]]@layers$counts)
  normalized_data <- as.data.frame(seurat_obj@assays[[i]]@layers$data)
